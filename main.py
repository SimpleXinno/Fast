...
ADMIN_ID = [6903379369]
CHECKER_API_URL = 'https://api-v2-production-669b.up.railway.app/shopify?site=https://juniperandspruce.myshopify.com&cc=5285460131620610|05|2027|471&proxy='
...
def get_main_menu_keyboard(user_id=None):
    buttons = [
        ...
        [Button.url(" Cʜᴀɴɴᴇʟ", "https://t.me/+QgNcqlLWUX41MjI1", style="success", icon=5445408306669582934)]
    ]
...
💡 Mᴀᴅᴇ ʙʏ @ITACHIxAURA"""
    ...
    filename = f"ITACHI{timestamp}.txt"
...
async def shopify_export_callback(event):
    ...
    if export_type == "charged":
        ...
        filename = f"charged_cards_@Itachixaurachkbot.txt"
        title = "CHARGED CARDS"
        emoji = "💎"
    else:
        cards_list = user_results.get('approved', [])
        filename = f"approved_cards_@Itachixaurachkbot.txt"
...
async def shopify_export_errors_callback(event):
    ...
    filename = f"errors_cards_@Itachixaurachkbot.txt"
    title = "ERROR CARDS"
    emoji = "⚠️"
